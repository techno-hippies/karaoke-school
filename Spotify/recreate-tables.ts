...
async function recreateTables() {
  console.log('🔄 Dropping and recreating Spotify tables with correct schema...');
  ...
  try {
    ...
    // Create album_image table (schema matches dump)
    await sql`
    ...
    await sql`CREATE INDEX idx_spotify_track_externalid_trackid ON spotify_track_externalid(trackid)`;
    await sql`CREATE INDEX idx_spotify_album_externalid_albumid ON spotify_album_externalid(albumid)`;
    
    console.log('Created all indexes');
    console.log('✅ All Spotify tables recreated with correct schema!');
