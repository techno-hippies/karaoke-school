 ...
 * - Added lensAccountAddress validation (cannot be zero address)
 * - Improved NatSpec documentation
 * - Maintained manual Ownable pattern for consistency with other contracts
 *
 * Design Philosophy:
...
contract ArtistRegistryV2 {
     ...
     * @dev Validates all inputs and checks for duplicate registrations
     */
    function registerArtist(
        ...
        if (lensAccountAddress == address(0)) revert InvalidAddress();
     ...
     *      V2: Fixed to prevent mapping overwrites when updating to existing PKP/handle
     */
    function updateArtist(
        ...
        // V2: Validate new values
        if (newPKPAddress == address(0)) revert InvalidPKPAddress();
        if (newLensAccountAddress == address(0)) revert InvalidAddress();
        if (bytes(newLensHandle).length == 0) revert InvalidLensHandle();

        // V2: Check for collisions on new PKP address
        ...
        // V2: Check for collisions on new Lens handle
        bytes32 newHandleHash = keccak256(bytes(newLensHandle));
        uint32 existingHandleMapping = lensHandleToGeniusId[newHandleHash];
        ...
        lensHandleToGeniusId[newHandleHash] = geniusArtistId;
