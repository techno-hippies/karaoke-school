...
# 2. Run specific step
bun run-unified.ts --step=download-audio --limit=20      # Audio download
bun run-unified.ts --step=separate-audio --limit=10      # Demucs separation
bun run-unified.ts --step=enhance-audio --limit=5        # Audio enhancement
...
Monetization via Unlock Protocol subscription locks (artists only):
```bash
# Deploy subscription locks for artists with PKP+Lens
bun src/processors/deploy-artist-unlock-locks.ts --limit=5
```

**Features:**
- **Artists only** - Requires `spotify_artist_id` (TikTok creators excluded)
- **Base Sepolia deployment** - ERC721 subscription locks
- **Fixed pricing** - $1.99 ETH per month, 30-day duration
- **Direct payments** - Funds flow directly to artist's PKP address
- **Prerequisites** - Artist must have PKP + Lens account already created

**Database Storage:**
- Lock info stored in `lens_accounts` table (subscription_lock_address, etc.)
- Query artists needing locks: `WHERE subscription_lock_address IS NULL`
...
| Processor | Name | Status Transition | Notes |
|-----------|------|-------------------|-------|
| `scrape-tiktok` | Scrape TikTok | `n/a → tiktok_scraped` | |
| `resolve-spotify` | Resolve Spotify | `tiktok_scraped → spotify_resolved` | |
| `discover-iswc` | ISWC Discovery | `spotify_resolved → iswc_found` | |
| `enrich-musicbrainz` | Enrich MusicBrainz | `iswc_found → metadata_enriched` | |
| `discover-lyrics` | Discover Lyrics | `metadata_enriched → lyrics_ready` | |
| `download-audio` | Download Audio | `lyrics_ready → audio_downloaded` | |
| `align-lyrics-forced` | Forced Alignment | `audio_downloaded → alignment_complete` | |
| `enrich-genius-legacy` | Genius Enrichment | `lyrics_ready+ → lyrics_ready` | |
| `translate-lyrics` | Lyrics Translation | `alignment_complete → translations_ready` | |
| `separate-audio` | Audio Separation | `translations_ready → stems_separated` | 🆕 |
| `select-segments` | Full-Song Segments | `stems_separated → segments_selected` | ✨ Simplified (0-190s) |
| `enhance-audio` | Audio Enhancement | `segments_selected → enhanced` | |
| `select-viral-clip` | Crop TikTok Clips | `enhanced → clips_cropped` | |
| `upload-videos-grove` | Upload TikTok Videos | `clips_cropped → clips_cropped` | |

### **🆕 New Processors (Web3 & Advanced):**
|| Processor | Purpose | Usage |
|-----------|---------|-------|
| `mint-pkps.ts` | Mint PKPs for artists & creators | `bun src/processors/mint-pkps.ts --limit=20` |
| `create-lens-accounts.ts` | Create Lens accounts for artists & creators | `bun src/processors/create-lens-accounts.ts --limit=20` |
| `deploy-artist-unlock-locks.ts` | Deploy Unlock subscription locks (artists only) | `bun src/processors/deploy-artist-unlock-locks.ts --limit=5` |
...
### 🎯 Segment Simplification (select-segments)
...
**Run**:
```bash
bun run-unified.ts --step=select-segments --limit=20  # Set segments for 20 tracks
```
...
bun src/processors/deploy-artist-unlock-locks.ts --limit=5  # Artists only, NOT creators
bun src/processors/transcribe-tiktok.ts --limit=10
```
...
bun run-unified.ts --step=discover-iswc --limit=1
...
bun run-unified.ts --step=discover-iswc --limit=1  # Try different step
```
...
bun run-unified.ts --step=discover-iswc --limit=100      # ISWC discovery
bun run-unified.ts --step=download-audio --limit=20      # Audio download (slower)
bun run-unified.ts --step=select-viral-clip --limit=50   # Crop TikTok clips
```
