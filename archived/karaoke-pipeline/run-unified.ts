 ...
 *
 * Usage:
 *   bun run-unified.ts [--step=PROCESSOR] [--limit=N] [--continuous] [--all]
 *   bun run-unified.ts --all --limit=50                      # All steps, max 50 tracks each
 *   bun run-unified.ts --step=download-audio --limit=10      # Only audio download, 10 tracks
 *   bun run-unified.ts --step=align-lyrics-forced --limit=5  # Only forced alignment, 5 tracks
...
function parseArgs(): RunOptions {
  ...
  for (const arg of args) {
    if (arg.startsWith('--step=')) {
      options.step = arg.split('=')[1];  // Now accepts processor names
