 ...
 * 
 * Usage:
 *   bun run scripts/validation/check-pipeline-integrity.ts
 */
...
async function checkPipelineIntegrity() {
  ...
  blockers.forEach(b => {
    const icon = b.blocker_reason === 'ready' ? '✅' : '❌';
    console.log(`  ${icon} ${b.blocker_reason}: ${b.count} videos`);
    
    if (b.blocker_reason !== 'ready' && b.count > 0) {
  ...
  if (recordingsWithoutWorks[0].count > 0) {
    issues.push({
      ...
      message: `${recordingsWithoutWorks[0].count} recordings minted without corresponding works`
    });
  } else {
