 ...
 * Only updates status to 'iswc_found' if track is in 'spotify_resolved' status
 * For tracks already past this point, just backfills the ISWC without changing status
 */
async function updatePipeline(track: Track, iswc: string | null, source: string): Promise<void> {
  await query(`
    ...
    SET
      status = CASE
        WHEN status = 'spotify_resolved' THEN 'iswc_found'
        ELSE status
      END,
      has_iswc = $1,
      iswc = $2,
...
export async function processISWCDiscovery(env: Env, limit: number = 50): Promise<void> {
  ...
  // Includes: tracks in spotify_resolved OR tracks without ISWC that aren't known failures
  const tracks = await query<Track>(`
    ...
    WHERE sp.isrc IS NOT NULL
      AND sp.has_iswc = false
      AND (sp.last_attempted_at IS NULL OR sp.last_attempted_at < NOW() - INTERVAL '1 hour')
      AND NOT EXISTS (
        SELECT 1 FROM iswc_lookup_failures f
        WHERE f.isrc = sp.isrc
          AND f.last_attempted_at > NOW() - INTERVAL '7 days'
      )
