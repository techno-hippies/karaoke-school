  ...
  if (uncachedTracks.length > 0) {
    ...
    for (const track of uncachedTracks) {
      try {
        ...
        // VALIDATION: Check if lyrics indicate instrumental track (after normalization/cleaning)
        const finalLyrics = normalizedLyrics || lrclibText || ovhText || '';
        const isInstrumental = (
          finalLyrics.length < 50 || // Very short lyrics
          /^\s*\[?\s*instrumental\s*\]?\s*$/i.test(finalLyrics.trim()) // Just says "Instrumental"
        );

        if (isInstrumental) {
          console.log(`     🎻 Detected instrumental track from lyrics content (${finalLyrics.length} chars)`);
          sqlStatements.push(`
            UPDATE song_pipeline
            SET has_lyrics = FALSE,
                error_stage = 'discover-lyrics',
                error_message = 'Instrumental track detected from lyrics content - no vocals for karaoke'
            WHERE spotify_track_id = '${track.spotify_track_id}'
          `);
          sqlStatements.push(
            logLyricsProcessingSQL(
              track.spotify_track_id,
              'failed',
              'Instrumental track detected - lyrics too short or marked as instrumental'
            )
          );
          failCount++;
