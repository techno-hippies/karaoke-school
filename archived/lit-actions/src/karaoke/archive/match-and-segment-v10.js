/**
 * Match and Segment v10: Add geniusArtistId for Genius API Decoupling
 *
 ...
 *
 * Changes from v9:
 * - Store geniusArtistId in contract for full Genius API decoupling
 * - Frontend can navigate to artist pages without API calls
 * - Uses new KaraokeCatalogV2 contract at 0xa3fE1628c6FA4B93df76e070fdCd103626D83039
 *
 * Time: ~5-10s
...
console.log('=== LIT ACTION v10 LOADED (geniusArtistId Support) ===');
...
const go = async () => {
  ...
  try {
    ...
    const artistId = song.primary_artist?.id || 0;
    ...
    console.log(`SoundCloud from Genius: ${soundcloudPermalink || 'NOT FOUND'}`);

    // If no SoundCloud URL in Genius, search sc.maid.zone for verified artist
    if (!soundcloudPermalink) {
      try {
        console.log('🔍 No SoundCloud in Genius, searching sc.maid.zone...');
        const searchQuery = encodeURIComponent(`${title} ${artist}`);
        const searchUrl = `https://sc.maid.zone/search?q=${searchQuery}&type=tracks`;
        ...
        if (searchResponse.ok) {
          ...
          // Parse top 3 track links
          const listingRegex = /<a\s+class="listing"\s+href="([^"]+)"/g;
          const matches = [];
          let match;
          while ((match = listingRegex.exec(html)) !== null && matches.length < 3) {
            matches.push(match[1]);
          }

          if (matches.length > 0) {
            console.log(`📋 Found ${matches.length} potential matches, checking for verified artists...`);

            // Check each result's artist profile for verification
            for (let i = 0; i < matches.length; i++) {
              const trackPath = matches[i];
              const username = trackPath.split('/')[1];
              console.log(`🔍 Checking result ${i + 1}: ${username}`);

              try {
                const profileUrl = `https://sc.maid.zone/${username}`;
                ...
                if (profileResponse.ok) {
                  ...
                  if (profileHtml.includes('<p style="color: var(--accent)">Verified</p>')) {
                    soundcloudPermalink = `https://sc.maid.zone${trackPath}`;
                    console.log(`✅ Found verified artist: ${username}`);
                    console.log(`✅ Using SoundCloud URL: ${soundcloudPermalink}`);
                    break;
                  } else {
                    console.log(`⚠️  Artist ${username} is not verified`);
                  }
                }
              } catch (profileError) {
                console.log(`⚠️  Error checking profile for ${username}: ${profileError.message}`);
                continue;
              }
            }

            if (!soundcloudPermalink) {
              console.log('⚠️  No verified artists found in top 3 results');
            }
          } else {
            console.log('⚠️  No results found on sc.maid.zone');
          }
        } else {
          console.log(`⚠️  sc.maid.zone search failed: HTTP ${searchResponse.status}`);
        }
      } catch (scError) {
        console.log(`⚠️  SoundCloud search error: ${scError.message}`);
        // Continue without SoundCloud URL
      }
    }
    ...
    if (soundcloudPermalink) {
      ...
      // Check Content-Length to detect 30-second snippets (< 600KB)
      try {
        ...
        console.log('⚠️  Audio size check failed:', checkError.message);
        hasFullAudio = true; // Assume full if check fails
      }
    } else {
...
8. Example: if Verse 1 goes from line 5 to line 12, return startLine: 5, endLine: 12.`;
    ...
    if (writeToBlockchain && result.isMatch && sections.length > 0) {
      try {
        ...
        // Song data struct for addFullSong (V10 - Added geniusArtistId)
        const songData = {
          id: songId,
          geniusId: geniusId,
        ...
        // ABI for addFullSong (V10 - 16 fields with geniusArtistId)
        const abi = [{
          "type": "function",
          "name": "addFullSong",
          "inputs": [{
