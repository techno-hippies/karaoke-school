/**
 * Match and Segment v9: SoundCloud Fallback + Song Structure Extraction
 *
 ...
 *
 * Changes from v8:
 * - SoundCloud fallback: If no SC link in Genius, searches sc.maid.zone for verified artist
 * - Checks top 3 results from sc.maid.zone for verified badge
 * - Check Content-Length header to detect 30-second clips (< 600KB = snippet)
...
console.log('=== LIT ACTION v9 LOADED (SC Fallback) ===');
...
8. Example: if Verse 1 goes from line 5 to line 12, return startLine: 5, endLine: 12.`;
    ...
    if (writeToBlockchain && result.isMatch && sections.length > 0) {
      try {
        ...
        // Song data struct for addFullSong (V4 - Added sectionsUri and alignmentUri)
        const songData = {
          id: songId,
          geniusId: geniusId,
          title: title,
        ...
        // ABI for addFullSong (V4 - 15 fields with sectionsUri and alignmentUri)
        const abi = [{
          "type": "function",
          "name": "addFullSong",
          "inputs": [{
