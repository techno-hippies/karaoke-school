...
import "../ArtistRegistryV2.sol";

/**
 * @title DeployArtistRegistryV2
 * @notice Deployment script for ArtistRegistryV2 on Base Sepolia
 *
 * V2 Changes:
 * - Fixed updateArtist() mapping collision bug
 * - Added lensAccountAddress validation
 ...
 */
contract DeployArtistRegistryV2 is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);

        console.log("=== ArtistRegistryV2 Deployment ===");
        console.log("Deployer:", deployer);
        console.log("Chain ID:", block.chainid);

        vm.startBroadcast(deployerPrivateKey);

        ArtistRegistryV2 registry = new ArtistRegistryV2();

        vm.stopBroadcast();

        console.log("\n=== Deployment Complete ===");
        console.log("ArtistRegistryV2 deployed to:", address(registry));
        ...
        console.log("\n=== V2 Improvements ===");
        console.log("- Fixed updateArtist() to prevent mapping collisions");
        console.log("- Added lensAccountAddress != address(0) validation");
        console.log("- Improved NatSpec documentation");
