...
async function main() {
  ...
  try {
    ...
    if (pkpBalanceBefore.lt(ethers.utils.parseEther('0.001'))) {
      console.warn('⚠️  WARNING: PKP balance is low! May not have enough gas.');
    }
    ...
    console.log('\n📝 Note: On-chain verification requires contract ABI (frontend will implement)');
    ...
    console.log(`\n💰 PKP Balance After: ${ethers.utils.formatEther(pkpBalanceAfter)} ETH`);
