...
var NeonDB = class {
  ...
  constructor(databaseUrl) {
    this.sql = Xs(databaseUrl);
  }
};

// src/processors/step-08-iswc-discovery.ts
async function processISWCDiscovery(env2, limit = 50) {
  console.log("\u{1F6AA} Step 8: ISWC Discovery (Gate)");
  const db = new NeonDB(env2.NEON_DATABASE_URL);
...
`);
  ...
  for (const track of tracks) {
    try {
      ...
      if (false) {
        apiCalls++;
        console.log(`   \u{1F50D} ${track.title} - calling Quansic API...`);
        try {
          const quansicResponse = await fetch(`${env2.QUANSIC_SERVICE_URL}/enrich-recording`, {
            method: "POST",
          ...
          console.error(`   \u274C ${track.title} - Quansic API call failed:`, error3.message);
        }
      }
      ...
      console.error(`   \u274C ${track.title} - Processing error:`, error3.message);
      await db.sql`
        UPDATE track_pipeline
        SET
          retry_count = retry_count + 1,
...
\u2705 Step 8 Complete:`);
  ...
  console.log(`   \u{1F4CA} Cache hits: ${cached}`);
  console.log(`   \u{1F310} API calls: ${apiCalls}`);
}
__name(processISWCDiscovery, "processISWCDiscovery");
async function checkCacheForISWC(db, isrc) {
...
__name(checkCacheForISWC, "checkCacheForISWC");

// src/processors/orchestrator.ts
async function runUnifiedPipeline(env2, options) {
  const limit = options?.limit || 50;
  const targetStep = options?.step;
  console.log("\u{1F680} Unified Pipeline Starting...");
  if (targetStep) {
...
${"=".repeat(60)}`);
    ...
    try {
      await step.processor(env2, limit);
      const duration = ((Date.now() - stepStart) / 1e3).toFixed(1);
      ...
      console.error(`\u274C Step ${step.number} failed:`, error3.message);
      results.push({
        step: step.number,
        name: step.name,
        success: false,
  ...
  console.log(`
${"=".repeat(60)}`);
  console.log(`\u{1F3C1} Pipeline Complete (${totalDuration}s)`);
...
__name(runUnifiedPipeline, "runUnifiedPipeline");

// src/routes.ts
var routes = new Hono2();
routes.post("/trigger", async (c) => {
  const step = c.req.query("step") ? parseInt(c.req.query("step")) : void 0;
...
var src_default = {
  async fetch(request, env2) {
    return app.fetch(request, env2);
  },
  async scheduled(event, env2) {
    console.log("\u23F0 Cron triggered:", (/* @__PURE__ */ new Date()).toISOString());
    try {
      await runUnifiedPipeline(env2, { limit: 50 });
    } catch (error3) {
      console.error("\u274C Cron failed:", error3.message);
    }
  }
};
...
// .wrangler/tmp/bundle-6csG21/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6csG21/middleware-loader.entry.ts
