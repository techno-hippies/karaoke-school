...
async function main() {
  ...
  try {
    ...
    const tracks = await sql`
      ...
      WHERE sa.grove_url IS NOT NULL
        AND NOT EXISTS (
          ...
          WHERE tm.spotify_track_id = sa.spotify_track_id 
          AND tm.instrumental_grove_cid IS NOT NULL
        )
    ...
    const webhookUrl = process.env.DEMUCS_WEBHOOK_URL || 'http://localhost:36949/webhooks/demucs-complete';
