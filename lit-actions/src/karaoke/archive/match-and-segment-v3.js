/**
 * Match and Segment v3: With ElevenLabs Forced Alignment
 *
 * Adds word-level timing using ElevenLabs forced alignment API.
 *
 ...
 * 4. Writes segments to KaraokeCatalogV1 contract on Base Sepolia
 * 5. Returns structured JSON with match decision + sections + alignment + txHash
 */

console.log('=== LIT ACTION v3 LOADED ===');
    ...
    if (runAlignment && result.isMatch && soundcloudPermalink && plainLyrics) {
      try {
        ...
        try {
          ...
          if (result.success) {
            // Success - extract alignment data
            alignment = {
