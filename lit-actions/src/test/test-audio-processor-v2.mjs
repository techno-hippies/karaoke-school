...
/**
 * Test Audio Processor v2 Lit Action (with Spleeter)
 *
 * Tests:
 * 1. Segment ownership verification via KaraokeCreditsV1 contract
 * 2. Spleeter audio separation (vocals + accompaniment)
 * 3. fal.ai accompaniment enhancement
 * 4. Credit-gated karaoke generation
 *
 * Prerequisites:
 ...
 * Usage:
 *   DOTENV_PRIVATE_KEY=40e9ed2b556418dc70af5b3512c03cd40b462872f444f71c18c35aedf9434d24 \
 *   dotenvx run -- bun run src/test/test-audio-processor-v2.mjs
 */
...
// Audio Processor v2 CID (Spleeter pipeline - fixed error handling)
const AUDIO_PROCESSOR_V2_CID = 'QmdEAmFbjuVWZWqvVbu4ekFtS38fUaF3GnknbhmjdyqvqH';

// Encrypted key path (only fal.ai needed, no ElevenLabs)
...
async function loadEncryptedKeys() {
  console.log('🔐 Loading encrypted API key...');
...
async function testAudioProcessor() {
  ...
  const { falApiKey } = await loadEncryptedKeys();
  ...
  const jsParams = {
    ...
    // fal.ai key (for accompaniment enhancement)
  ...
  console.log('\n🚀 Executing Audio Processor v2 Lit Action (Spleeter Pipeline)...');
  console.log('⏱️  Expected time: ~30s for full pipeline');
  console.log('   Step 1: Verify segment ownership on-chain');
  console.log('   Step 2: Decrypt fal.ai API key');
  console.log('   Step 3: Verify audio availability');
  console.log('   Step 4: Process with Modal Spleeter (download + trim + vocals/accompaniment)');
  console.log('   Step 5: Enhance accompaniment (instrumental) with fal.ai');
  console.log('   Step 6: Prepare for Grove upload and registry update');
  ...
  try {
    ...
    const outputFile = join(outputDir, `audio-processor-v2-result.json`);
    ...
    console.log('Enhanced Accompaniment MP3:', (response.assets.accompanimentSize / 1024 / 1024).toFixed(2) + 'MB');

    console.log('\n--- Processing Time ---');
    console.log('Modal Spleeter (download + trim + stems):', response.processing.modal.toFixed(1) + 's');
    console.log('fal.ai (accompaniment enhancement):', response.processing.falai.toFixed(1) + 's');
    ...
    console.log('\n--- URLs ---');
    console.log('Enhanced Accompaniment:', response.urls.enhancedAccompaniment);
...
// Run test
console.log('🎹 Audio Processor v2 Test (Spleeter Pipeline)\n');
console.log('Testing:');
console.log('  1. Segment ownership verification (KaraokeCreditsV1 on Base Sepolia)');
console.log('  2. Modal Spleeter endpoint (download + trim + vocals/accompaniment)');
console.log('  3. fal.ai accompaniment (instrumental) enhancement');
console.log('  4. Grove upload preparation');
console.log('  5. KaraokeCatalog registry update preparation');
console.log('\nChanges from v1:');
console.log('  ✅ Spleeter instead of Demucs (2x faster)');
console.log('  ✅ Accompaniment instead of drums (full instrumental)');
console.log('  ✅ No ElevenLabs (alignment done in match-and-segment-v5)');
console.log('  ✅ Simplified for <30s timeout\n');
