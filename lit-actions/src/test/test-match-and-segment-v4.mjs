...
/**
 * Test Script for Match and Segment v4 Lit Action
 *
 * Tests the complete pipeline with ElevenLabs forced alignment organized into lines:
 ...
 *
 * Usage:
 *   bun run src/test/test-match-and-segment-v4.mjs
 *   bun run src/test/test-match-and-segment-v4.mjs [genius-id]
 */
...
// CID for match-and-segment-v4 (line-based alignment with word-level timing per line)
const MATCH_AND_SEGMENT_V4_CID = 'QmZsQkNUXej4F46uCzD6WzHdSbBjRJXau7wsJgFysMg9hQ';
...
async function main() {
  console.log('🎤 Match and Segment v4 Test (Line-Based ElevenLabs Alignment)\n');
  ...
  try {
    ...
    const authManager = createAuthManager({
      storage: storagePlugins.localStorageNode({
        appName: "match-and-segment-v4-test",
    ...
    console.log('\n🚀 Executing Lit Action v4...');
    console.log('⏱️  Expected time: ~29s (with ElevenLabs line-based alignment)');
    const startTime = Date.now();

    try {
      const result = await litClient.executeJs({
        ipfsId: MATCH_AND_SEGMENT_V4_CID,
      ...
      const outputFile = join(outputDir, `song-${geniusId}-v4-result.json`);
      ...
      console.log('\n--- ElevenLabs Alignment (Line-Based) ---');
      if (response.alignment) {
        console.log(`✅ Alignment completed (line-based structure)`);
        ...
        console.log(`\nLine Preview (first 5 lines with timing):`);
        if (response.alignment.lines && response.alignment.lines.length > 0) {
          response.alignment.lines.forEach((line, i) => {
            console.log(`  ${i + 1}. ${line.start}s - ${line.end}s: "${line.text.substring(0, 50)}${line.text.length > 50 ? '...' : ''}"`);
          });
        }
