...
/**
 * Test Script for Match and Segment v6 Lit Action
 *
 * Tests the fast pipeline WITHOUT ElevenLabs alignment:
 * - Genius API → Song metadata
 * - LRClib API → Synced lyrics
 * - Flash 2.5 Lite → Match + segmentation (NO translations)
 * - Contract write IMMEDIATELY (no alignment required)
 *
 * Expected time: ~5-10s (was ~30s with alignment)
 *
 * Usage:
 *   bun run src/test/test-match-and-segment-v6.mjs
 *   bun run src/test/test-match-and-segment-v6.mjs [genius-id]
 */
...
// Encrypted key paths (v10 = encrypted for match-and-segment v6 with runOnce)
const OPENROUTER_KEY_PATH = join(__dirname, '../karaoke/keys/openrouter_api_key_v10.json');
const GENIUS_KEY_PATH = join(__dirname, '../karaoke/keys/genius_api_key_v10.json');
...
async function main() {
  console.log('🎤 Match and Segment v6 Test (Fast - No Alignment)\n');
  console.log('━'.repeat(80));
  console.log('\nThis test will:');
  console.log('1. Fetch song metadata from Genius');
  console.log('2. Get synced lyrics from LRClib');
  console.log('3. Match songs + segment with AI (NO translations)');
  console.log('4. Write to blockchain IMMEDIATELY (no alignment required)');
  console.log('5. Verify song exists in contract');
  console.log('\n━'.repeat(80));
  ...
  try {
    ...
    const authManager = createAuthManager({
      storage: storagePlugins.localStorageNode({
        appName: "match-and-segment-v6-test",
    ...
    console.log('\n🚀 Executing Lit Action v6...');
    console.log('⏱️  Expected time: ~5-10s (much faster without alignment!)');
    const startTime = Date.now();

    // Use IPFS CID for production (with runOnce for LLM call)
    const MATCH_AND_SEGMENT_V6_CID = 'QmfXchQoiqJGu1ia61X52pdSuVFqR51kCD4PUvRagJHk6y';

    try {
      const result = await litClient.executeJs({
        ipfsId: MATCH_AND_SEGMENT_V6_CID,
      ...
      const outputFile = join(outputDir, `song-${geniusId}-v6-result.json`);
      ...
      console.log(`   v5 (with ElevenLabs): ~30-60s + ~$0.05`);
      console.log(`   v6 (fast structure):  ~${(executionTime / 1000).toFixed(1)}s + ~$0.01`);
      console.log(`   Speedup: ${((30 / (executionTime / 1000))).toFixed(1)}x faster!`);
      console.log('━'.repeat(80));
