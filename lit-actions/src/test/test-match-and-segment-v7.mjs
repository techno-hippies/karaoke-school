...
/**
 * Test Script for Match and Segment v7 Lit Action
 *
 * Tests v7 with hardcoded system PKP credentials:
 ...
 *
 * Usage:
 *   bun run src/test/test-match-and-segment-v7.mjs
 *   bun run src/test/test-match-and-segment-v7.mjs [genius-id]
 */
...
// Encrypted key paths (v13 = encrypted for match-and-segment v7 fixed with metadata upload)
const OPENROUTER_KEY_PATH = join(__dirname, '../karaoke/keys/openrouter_api_key_v13.json');
const GENIUS_KEY_PATH = join(__dirname, '../karaoke/keys/genius_api_key_v13.json');
...
async function main() {
  console.log('🎤 Match and Segment v7 Test (Hardcoded System PKP)\n');
  ...
  console.log('4. Write to blockchain with hardcoded system PKP');
  ...
  try {
    ...
    const authManager = createAuthManager({
      storage: storagePlugins.localStorageNode({
        appName: "match-and-segment-v7-test",
    ...
    console.log('\n🚀 Executing Lit Action v7...');
    console.log('⏱️  Expected time: ~5-10s');
    const startTime = Date.now();

    // Use IPFS CID for v7 (fixed - with metadata upload)
    const MATCH_AND_SEGMENT_V7_CID = 'QmVtM6ScW31n7UMw6TmD3HeQZvupn6YvefE4sAmenxYYt5';
    ...
    try {
      ...
      const outputFile = join(outputDir, `song-${geniusId}-v7-result.json`);
