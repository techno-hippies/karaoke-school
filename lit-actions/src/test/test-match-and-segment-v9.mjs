...
/**
 * Test Script for Match and Segment v9 Lit Action
 *
 * Tests v9 with SoundCloud fallback (sc.maid.zone verified artist search):
 * - System PKP hardcoded in IPFS code (immutable, can't be spoofed)
 * - If no SoundCloud in Genius, searches sc.maid.zone for verified artist
 ...
 *
 * Usage:
 *   bun run src/test/test-match-and-segment-v9.mjs
 *   bun run src/test/test-match-and-segment-v9.mjs [genius-id]
 */
...
async function main() {
  console.log('🎤 Match and Segment v9 Test (SC Fallback + File Size Detection)\n');
  ...
  try {
    ...
    const authManager = createAuthManager({
      storage: storagePlugins.localStorageNode({
        appName: "match-and-segment-v9-test",
    ...
    console.log('\n🚀 Executing Lit Action v9...');
    console.log('⏱️  Expected time: ~5-10s');
    const startTime = Date.now();

    // Use IPFS CID for v9 (V4 ABI with 15 fields + SC fallback)
    const MATCH_AND_SEGMENT_V9_CID = 'QmQtXQCMSjaeD7jCgvH6u7cnq2mFnucLieB3CaEsSA9HjN';

    try {
      const result = await litClient.executeJs({
        ipfsId: MATCH_AND_SEGMENT_V9_CID,
      ...
      const outputFile = join(outputDir, `song-${geniusId}-v9-result.json`);
