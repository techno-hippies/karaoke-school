/**
 * Karaoke Grader v12 - Line-Level FSRS (All params properly normalized)
 *
 ...
 *
 * CRITICAL FIX: Proper type normalization for all 7 parameters
 * - performanceId: BigNumber (uint256)
 * - lineId: hexZeroPad to 32 bytes (bytes32)
 * - segmentHash: hexZeroPad to 32 bytes (bytes32)
 * - lineIndex: BigNumber (uint16)
 * - performer: getAddress() for proper checksummed address
 * - score: BigNumber (uint16)
 * - metadataUri: string (unchanged)
 *
 * Flow:
 * 1. Validate lineId is provided and non-zero
 * 2. Transcribe user audio via Voxtral STT
 * 3. Calculate pronunciation score
 * 4. Submit score via gradeLinePerformance() (7 params, all properly typed)
 * 5. Emit LinePerformanceGraded event for FSRS line-level tracking
 */

// ============================================================
// CONTRACT CONFIGURATION
// ============================================================
const PERFORMANCE_GRADER_ADDRESS = '0x5da966F19bD1a67D6AAda68b338e4B336CeA5aE8';
...
const go = async () => {
  ...
  try {
    ...
    if (testMode) {
      ...
      Lit.Actions.setResponse({
        response: JSON.stringify({
          ...
          version: 'v12-final-working-contract',
    ...
    // Try to submit to PerformanceGrader contract
    ...
    try {
      txHash = await submitToPerformanceGrader({
        ...
        lineId,  // Already validated above, no fallback needed
        segmentHash,
...
// ============================================================
async function submitToPerformanceGrader({ performanceId, lineId, segmentHash, lineIndex, performer, score, metadataUri }) {
  const provider = new ethers.providers.JsonRpcProvider(LENS_TESTNET_RPC);

  // Create contract instance with line-level grading support
  const performanceGraderABI = [
    'function gradeLinePerformance(uint256 performanceId, bytes32 lineId, bytes32 segmentHash, uint16 lineIndex, address performer, uint16 score, string metadataUri) external',
    'event LinePerformanceGraded(uint256 indexed performanceId, bytes32 indexed lineId, bytes32 indexed segmentHash, uint16 lineIndex, address performer, uint16 score, string metadataUri, uint64 timestamp)'
  ];
  ...
  // Encode function call for line-level grading
  // CRITICAL: Ensure all parameters are properly typed
  const performanceIdBN = ethers.BigNumber.from(performanceId); // uint256
  ...
  const performerAddress = ethers.utils.getAddress(performer); // address (checksummed)
  const scoreUint16 = ethers.BigNumber.from(score); // uint16

  const gradePerformanceTxData = contract.interface.encodeFunctionData('gradeLinePerformance', [
    performanceIdBN,
    lineIdBytes32,
    segmentHashBytes32,
    lineIndexUint16,
  ...
  try {
    const simResult = await provider.call({
      ...
      data: gradePerformanceTxData,
      gasLimit: ethers.utils.hexlify(2000000),
      value: '0x0',
    });
    console.log('✅ Simulation succeeded:', simResult);
  } catch (simError) {
    console.error('❌ Simulation failed with reason:', simError.reason || simError.message || simError);
