 ...
 *   bun tests/karaoke/test-karaoke-grader.mjs
 */
const fs = require('fs');
...
const LIT_ACTION_CID = cids.karaoke; // process.env.KARAOKE_GRADER_CID
...
async function createAuthContext(litClient) {
  const authManager = createAuthManager({
    storage: storagePlugins.localStorageNode({
      ...
      storagePath: `./lit-auth-storage-${LIT_NETWORK}-` + Math.random().toString(16).slice(2, 8)
    })
  });
...
console.log('Audio Path:', AUDIO_PATH);
  ...
  try {
    ...
    const assertions = [
      ...
      {
        name: 'Tx hash only when testMode=false and skipTx=false',
