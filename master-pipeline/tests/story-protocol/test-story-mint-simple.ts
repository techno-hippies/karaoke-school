...
// Test 1: Mint to our own wallet with swiftysavvy metadata
...
console.log('📝 Test: Minting swiftysavvy metadata to OUR WALLET');
...
try {
  const response = await client.ipAsset.registerIpAsset({
    nft: {
      type: "mint",
      spgNftContract,
      recipient: account.address, // Mint to ourselves (not Lens account)
    },
    ipMetadata: {
