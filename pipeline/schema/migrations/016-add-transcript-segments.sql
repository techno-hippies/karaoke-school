...
-- Purpose: Store hybrid STT segments with word-level timestamps for karaoke overlay in app
...
'Hybrid STT segments with word-level timing. Format: [{ text: string, start: number, end: number, words: [{ word: string, start: number, end: number }] }]';
