...
const { values } = parseArgs({
  ...
  options: {
    ...
    limit: { type: 'string' }, // Optional - for testing. Default: generate all available
  },
  strict: true,
});
...
/**
 * Check if a line is purely an ad lib (background vocals)
 * Examples: "(Ooh)", "(Ooh, yeah, ooh, yeah)", "(Let him go)"
 */
function isPureAdLib(text: string): boolean {
  return /^\s*\([^)]+\)\s*$/.test(text);
}

/**
 * Strip trailing ad libs from lyrics for exercise generation
 * "I'm just a poor boy (Ooh, poor boy)" → "I'm just a poor boy"
 * "Any way the wind blows (Ooh)" → "Any way the wind blows"
 */
...
async function main() {
  ...
  // No limit by default - generate all available exercises
  ...
  if (exerciseType === 'all' || exerciseType === 'translation') {
    ...
    for (const enLine of enLyrics) {
      ...
      // Skip pure ad libs like "(Ooh)" or "(Let him go)"
      if (isPureAdLib(enLine.text)) continue;

      // Strip trailing ad libs for exercise text
      ...
      try {
        // Use cleaned text (without ad libs) for exercise generation
  ...
  if (exerciseType === 'all' || exerciseType === 'sayitback') {
    ...
    for (const enLine of enLyrics) {
      ...
      // Skip very short lines after stripping (less than 2 words)
      const wordCount = cleanText.split(/\s+/).length;
