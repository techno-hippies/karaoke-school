...
const { litNetwork, networkModule, networkName } = getLitNetworkConfig();
 ...
 */
async function findClipsToEncrypt(limit: number, litNetworkFilter: string): Promise<ClipToEncrypt[]> {
  const sql = `
    SELECT
      ks.spotify_track_id,
    ...
    JOIN pkp_accounts pkp ON pkp.pkp_address = la.pkp_address
    WHERE ks.fal_enhanced_grove_url IS NOT NULL
      AND ks.encrypted_full_cid IS NULL
      AND la.subscription_lock_address IS NOT NULL
      AND pkp.network = $2
      AND (ks.lit_network IS NULL OR ks.lit_network = $2)
    ORDER BY ks.created_at DESC
    LIMIT $1
  `;

  return await query<ClipToEncrypt>(sql, [limit, litNetworkFilter]);
}
...
async function initLitClient(): Promise<any> {
  ...
  console.log(`   🔐 Initializing Lit Protocol client (${networkName})...`);

  const litClient = await createLitClient({
    network: networkModule as any, // Type assertion for network module compatibility
  });
...
async function processClip(
  ...
  try {
    if (clip.pkp_network !== litNetwork) {
      throw new Error(`PKP network ${clip.pkp_network} does not match active Lit network ${litNetwork}`);
    }
...
async function main() {
  ...
  const clips = await findClipsToEncrypt(limit, litNetwork);
