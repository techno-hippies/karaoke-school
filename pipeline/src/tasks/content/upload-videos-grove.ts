 ...
 */
export async function processUploadGroveVideos(limit = 10, videoId?: string): Promise<void> {
  const env = {
  ...
  const videos = await query<VideoToUpload>(`
    SELECT
      v.video_id,
      v.creator_username,
      FALSE as is_copyrighted,
      NULL as pipeline_status,
      v.spotify_track_id
    FROM tiktok_videos v
    WHERE v.grove_video_cid IS NULL
    ${videoId ? 'AND v.video_id = $2' : ''}
    ORDER BY v.video_id
    LIMIT $1
  `, videoId ? [limit, videoId] : [limit]);
...
if (import.meta.main) {
  ...
  const videoId = args.find(arg => arg.startsWith('--videoId='))?.split('=')[1];

  processUploadGroveVideos(limit, videoId)
    .then(() => {
