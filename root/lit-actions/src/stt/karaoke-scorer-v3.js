/**
 * Karaoke Scorer v3 - Simplified Architecture
 *
 * SECURITY FIX: Reads lyrics from on-chain ClipRegistry (prevents spoofing)
 * SIMPLIFICATION: Hardcodes public contract addresses (no unnecessary encryption)
 *
 * Flow:
...
const go = async () => {
  ...
  try {
    ...
    // Decrypt Voxstral API key (the only secret)
    ...
    // Query ClipRegistry for clip metadata (using hardcoded address)
    ...
    // Submit score to scoreboard contract (using hardcoded address)
    try {
      ...
      await Lit.Actions.runOnce({ waitForResponse: true, name: analyticsRunOnceName }, async () => {
        ...
        const analyticsData = {
          ...
          metadata: JSON.stringify({
            ...
            scoreboard_address: SCOREBOARD_CONTRACT_ADDRESS
          })
        };
