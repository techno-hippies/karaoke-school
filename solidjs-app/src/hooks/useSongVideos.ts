/**
 * useSongVideos - Fetch all creator videos for a song by Spotify track ID
 * SolidJS implementation using @tanstack/solid-query
 */
 ...
 */
export function useSongVideos(spotifyTrackId: Accessor<string | undefined>) {
  return createQuery(() => ({
    ...
    queryFn: async () => {
      ...
      const filteredItems = allResult.value.items.filter((post: any) => {
        ...
        return attrs.some((a: any) => a.key === 'spotify_track_id' && a.value === trackId)
      })
